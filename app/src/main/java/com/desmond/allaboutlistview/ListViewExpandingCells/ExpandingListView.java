...
public class ExpandingListView extends ListView {
        ...
        observer.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
                ...
                animations.add(ObjectAnimator.ofFloat(
                        view.findViewById(R.id.expanding_layout), "alpha", 0, 1));
    ...
    private void collapseView(final View view) {
        ...
        view.setLayoutParams(new LayoutParams(
                LayoutParams.MATCH_PARENT, viewObject.getCollapsedHeight()));
     ...
     * collapse upwards (leaving top bound untouched). If the change in bounds results
     * in the complete disappearance of a cell, its upper bound is moved to the top
     * of the screen so as not to hide any additional content that the user has not
     * interacted with yet. Furthermore, if the collapsed cell is partially off screen
     * when it is first clicked, it is translated such that its full contents are visible.
    ...
    private int[] getTopAndBottomTranslations(int top, int bottom, int yDelta, boolean isExpanding) {
        ...
        if (isExpanding) {
            ...
            isCollapsingBelowBottom = ((yTranslationTop + yTranslationBottom <= range) &&
                    isCollapsingBelowBottom);

            if (isCollapsingBelowBottom) {
                yTranslationTop = yTranslationBottom - leftOverExtent;

                // Hack to force the view not shift the top bound when it is visible
                yTranslationTop = yTranslationTop < range ? 0 : yTranslationTop;
                yTranslationBottom = yDelta - yTranslationTop;
            }
